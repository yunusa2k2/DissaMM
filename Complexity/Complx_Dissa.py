...
def benchmark_model(model, dummy_input, precision="fp32", runs=50):
    ...
    if precision == "fp16":
        ...
        # Dynamic quantization (only supported on CPU)
    ...
    # Warmup
    for _ in range(50):  # more warmup for FP16
        _ = model(dummy_input)
    ...
    fps = dummy_input.size(0) / avg_time  # correct FPS for batch size
    ...
    return avg_time * 1000, fps, mem  # ms, FPS, MB

num_classes = 10
model = Dissa(num_classes=num_classes).to(device)
...
batch_size = 8  # realistic batch size for FP16 acceleration
