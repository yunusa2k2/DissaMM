...
# -*- coding: utf-8 -*-
"""
Created on Sat Sep  6 15:18:24 2025
...
input_dim = 25088  # VGG16 feature size
...
state_dict = torch.load("vgg16_mlp_damage.pth", map_location=device, weights_only=True)
...
def perturb_image(img_np, superpixels, mask):
    ...
    for seg_id, keep in enumerate(mask):
        if keep == 0:
            perturbed[superpixels == seg_id] = 128  # gray
    return perturbed
