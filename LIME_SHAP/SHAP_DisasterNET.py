...
# MLP classifier
input_dim = 25088  # VGG16 output features (7*7*512)
mlp = nn.Sequential(
...
mlp.load_state_dict(torch.load("vgg16_mlp_relevancy.pth", map_location=device))
mlp.eval()

# Combined model: image -> VGG16 features -> MLP
...
model = CombinedModel(vgg16, mlp).to(device)
model.eval()
